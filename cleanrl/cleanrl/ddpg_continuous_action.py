...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="duckietown",
        help="the wandb's project name")
    parser.add_argument("--wandb-entity", type=str, default='duckietown',
        help="the entity (team) of wandb's project")
    parser.add_argument("--capture-video", type=lambda x: bool(strtobool(x)), default=False, nargs="?", const=True,
        help="whether to capture videos of the agent performances (check out `videos` folder)")

    # Algorithm specific arguments
    parser.add_argument("--env-id", type=str, default=None,#"HopperBulletEnv-v0",
...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = PyTorchObsWrapper(env)  # to make the images from 160x120x3 into 3x160x120
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        for idx, d in enumerate(dones):
            if d:
                print(infos[idx]['episode'].keys())
                #real_next_obs[idx] = infos[idx]["terminal_observation"]
                real_next_obs[idx] = infos[idx]["episode"]["terminal_observation"]
